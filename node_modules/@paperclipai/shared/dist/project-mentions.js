...
export function buildUserMentionHref(userId) {
    return `${USER_MENTION_SCHEME}${userId.trim()}`;
}
export function parseUserMentionHref(href) {
    if (!href.startsWith(USER_MENTION_SCHEME))
        return null;
    let url;
    try {
        url = new URL(href);
...
export function buildRoutineMentionHref(routineId) {
    return `${ROUTINE_MENTION_SCHEME}${routineId.trim()}`;
}
export function parseRoutineMentionHref(href) {
    if (!href.startsWith(ROUTINE_MENTION_SCHEME))
        return null;
    let url;
    try {
        url = new URL(href);
...
export function extractUserMentionIds(markdown) {
    if (!markdown)
        return [];
    const ids = new Set();
...
export function extractRoutineMentionIds(markdown) {
    if (!markdown)
        return [];
    const ids = new Set();
