...
const PAPERCLIP_BASE = path.join(
  os.homedir(),
  ".paperclip",
  "instances",
  "default",
...
function getHermesHomePath(companyId: string, agentId: string): string {
...
export async function handleAgentEvent(
  ...
  try {
    ...
    ctx.logger.error(
      `Failed to provision hermes-home for agent ${payload.agentId}: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
}
